...
function layout.BankBar.BankBar:setupButtons()
    ...
    Part.Cursor.setCursorPos(button_x_end, button_y)
    Part.Cursor.incCursor(5, 0)

    -- copy button
    Part.Cursor.setCursorSize(self.button_w, self.button_h * 2 + Part.Cursor.getCursorPadY())
...
function layout.ConfigBar.ConfigBar:setupButtons()
    ...
    -- x offset for file handling and reset buttons
    local pos_file_handling_x = 230
    local pos_reset_x = 320
    ...
    Part.Cursor.setCursorSize(40, self.button_h)
    ...
    -- reset values button
    Part.Cursor.setCursorPos(source_x + pos_reset_x, source_y)
    Part.Cursor.setCursorSize(60, 35)

    -- reset values button
    button = Part.Control.Config.HardReset.ButtonConfigResetHard:new(nil, self.handler, "Reset Config")
