...
function gui.Hint.Hint:buffer()
    ...
    -- output block width
    local width_limit = w -- maximum width per line
    ...
    gfx.setimgdim(self.buffer_slot, -1, -1)
    gfx.setimgdim(self.buffer_slot, block_out_width, block_out_height)

    -- set target buffer slot
    gfx.dest = self.buffer_slot

    -- draw background
    Part.Draw.Graphics.drawRectangle(0, 0, block_out_width, block_out_height,
        Part.Color.Lookup.color_palette.hint.stage_bg, nil)
