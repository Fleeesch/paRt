...
function draw.Graphics.drawInfoBar()
    ...
    -- bank label
    label_x = x + Part.Functions.rescale(10)
    label_y = y + h + Part.Functions.rescale(7)
    place_label(label_x, label_y, "Bank")
    place_frame(label_x + Part.Functions.rescale(30), y + h + Part.Functions.rescale(2), 175, 45)
    label_x = label_x + Part.Functions.rescale(3)
    label_y = label_y + Part.Functions.rescale(9)
    draw.Graphics.setFont(24)
    place_label(label_x, label_y, "+")
    draw.Graphics.setFont(12)

    -- config
    label_x = x + Part.Functions.rescale(260)
    label_y = y + h + Part.Functions.rescale(20)
    place_label(label_x, label_y, "Config")
    place_frame(label_x + Part.Functions.rescale(40), y + h + Part.Functions.rescale(2), 175, 45)

    -- config from file
    if Part.Global.js_extension_available then
        label_x = x + Part.Functions.rescale(485)
        label_y = y + h + Part.Functions.rescale(14)
        place_label(label_x, label_y, "Config\nfrom file")
        place_frame(label_x + Part.Functions.rescale(45), y + h + Part.Functions.rescale(2), 70, 45)
    end
end
