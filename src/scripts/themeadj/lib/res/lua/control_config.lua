...
-- -------------------------------------------
config.ButtonConfig = Part.Control.Control:new()

function config.ButtonConfig:new(o, handler, text)
...
--  Control : Config Button : Reset Submit
-- -------------------------------------------

function config.ButtonConfig:resetSubmit()
    self.submit_counter = 0
end

--  Control : Config Button : Use Submit
-- -------------------------------------------

function config.ButtonConfig:useSubmit()
    self.use_submit = true
end

--  Control : Config Button : Set Font Flags
-- -------------------------------------------

function config.ButtonConfig:setFontFlags(flags)
    self.font_flags = flags
end

--  Control : Config Button : Set Color
-- -------------------------------------------

function config.ButtonConfig:setColor(fg, bg, fg_on, bg_on, border)
    -- foreground
...
--  Control : Config Button : Update State
-- -------------------------------------------

function config.ButtonConfig:updateState()

end

--  Control : Config Button : Submit Pending
-- -------------------------------------------

function config.ButtonConfig:submitPending()
...
--  Control : Config Button : Submit
-- -------------------------------------------

function config.ButtonConfig:submit()
...
--  Control : Config Button : Action
-- -------------------------------------------

function config.ButtonConfig:action(press)

end

--  Control : Config Button : Highlight
-- -------------------------------------------

function config.ButtonConfig:highlight()
    self.highlight_counter = 1
end

--  Control : Config Button : Get Output Text
-- -------------------------------------------

function config.ButtonConfig:getOutputText()
    return self.text
end

--  Control : Config Button : Prepare
-- -------------------------------------------

function config.ButtonConfig:prepare()
...
--  Control : Config Button : Draw
-- -------------------------------------------

function config.ButtonConfig:draw()
    -- get dimensions
    local x = self.draw_x
    local y = self.draw_y
    local w = self.draw_w
...
-- ===============================================================================
--                      Control : Config : Hard Reset
-- ===============================================================================


--  Control : Config Button Reset Hard
-- -------------------------------------------
config.HardReset.ButtonConfigResetHard = config.ButtonConfig:new()

function config.HardReset.ButtonConfigResetHard:new(o, handler, text)
    o = o or config.ButtonConfig:new(o, handler, text)
    setmetatable(o, self)
    self.__index = self
...
function config.HardReset.ButtonConfigResetHard:getOutputText()
    if self:submitPending() then
        return "Sure?"
    end
...
function config.HardReset.ButtonConfigResetHard:submit()
...
--  Control : Config Button Reset : Action
-- -------------------------------------------

function config.HardReset.ButtonConfigResetHard:action()
    -- if self.handler:copyModeActive() then
    --     return
    -- end
...
-- ===============================================================================
--                      Control : Config : Load Default Config
-- ===============================================================================


--  Control : Config Button Reset
-- -------------------------------------------
config.LoadDefault.ButtonConfigLoadDefault = config.ButtonConfig:new()

function config.LoadDefault.ButtonConfigLoadDefault:new(o, handler, text)
    o = o or config.ButtonConfig:new(o, handler, text)
    setmetatable(o, self)
    self.__index = self
...
function config.LoadDefault.ButtonConfigLoadDefault:getOutputText()
    if self:submitPending() then
        return "Sure?"
    end
...
function config.LoadDefault.ButtonConfigLoadDefault:submit()
    -- if self.handler:copyModeActive() then
    --     return
    -- end
...
function config.LoadDefault.ButtonConfigLoadDefault:action()
    -- if self.handler:copyModeActive() then
    --     return
    -- end
...
-- ===============================================================================
--                      Control : Config File Button
-- ===============================================================================

--  Control : Config File Button
-- -------------------------------------------
config.Abstract.ButtonConfigFile = config.ButtonConfig:new()

function config.Abstract.ButtonConfigFile:new(o, handler, text, target_file)
    o = o or config.ButtonConfig:new(o, handler, text)
    setmetatable(o, self)
    self.__index = self

    o.target_file = target_file or ""
    o.use_browser = target_file == nil

    o.use_selected_config = false


    return o
end

--  Control : Config File Button : Use Selected Config
-- -------------------------------------------------------------
function config.Abstract.ButtonConfigFile:useSelectedConfig()
    self.use_selected_config = true
end

--  Control : Config File Button : Action
-- -------------------------------------------

function config.Abstract.ButtonConfigFile:action()
end
...
--  Control : Config Button Save
-- -------------------------------------------
config.Save.ButtonConfigSave = config.Abstract.ButtonConfigFile:new()

function config.Save.ButtonConfigSave:new(o, handler, text, target_file)
    o = o or config.Abstract.ButtonConfigFile:new(o, handler, text, target_file)
    setmetatable(o, self)
    self.__index = self
...
--  Control : Config Button Save : Action
-- -------------------------------------------

function config.Save.ButtonConfigSave:action()
    self.handler:saveConfig(self.use_selected_config)
end
...
--  Control : Config Button Load
-- -------------------------------------------
config.Load.ButtonConfigLoad = config.Abstract.ButtonConfigFile:new()

function config.Load.ButtonConfigLoad:new(o, handler, text, target_file)
    o = o or config.Abstract.ButtonConfigFile:new(o, handler, text, target_file)
    setmetatable(o, self)
    self.__index = self
...
--  Control : Config Button Load : Action
-- -------------------------------------------

function config.Load.ButtonConfigLoad:action()
    self.handler:loadConfig(self.use_selected_config)
end

-- ===============================================================================
--                      Control : Config : Select
-- ===============================================================================


--  Control : Bank Button Select
-- -------------------------------------------
config.SelectConfig.ButtonConfigSelect = config.ButtonConfig:new()

function config.SelectConfig.ButtonConfigSelect:new(o, handler, text, config_index)
    o = o or config.ButtonConfig:new(o, handler, text)
    setmetatable(o, self)
    self.__index = self

    o.config_index = config_index

    o.copy_target = false

    table.insert(Part.List.control_button_config_select, o)
...
function config.SelectConfig.ButtonConfigSelect:updateState()
    self.state = self.handler.config_selected == self.config_index
end

--  Control : Bank Button Select : Action
-- -------------------------------------------

function config.SelectConfig.ButtonConfigSelect:action()
    self.handler:selectConfig(self.config_index)
end
...
-- -------------------------------------------
config.Copy.ButtonBankCopy = config.ButtonConfig:new()

function config.Copy.ButtonBankCopy:new(o, handler, text)
    o = o or config.ButtonConfig:new(o, handler, text)
    setmetatable(o, self)
    self.__index = self
...
function config.Copy.ButtonBankCopy:updateState()
...
function config.Copy.ButtonBankCopy:action()
...
--  Control : Bank Button Select
-- -------------------------------------------
config.SelectBank.ButtonBankSelect = config.ButtonConfig:new()

function config.SelectBank.ButtonBankSelect:new(o, handler, text, bank)
    o = o or config.ButtonConfig:new(o, handler, text)
    setmetatable(o, self)
    self.__index = self
    ...
    o.color_select_bg_on = Part.Functions.deepCopy(Part.Color.Lookup.color_palette.color.blue)
...
function config.SelectBank.ButtonBankSelect:colorsDefault()
...
function config.SelectBank.ButtonBankSelect:colorsCopy()
...
function config.SelectBank.ButtonBankSelect:clearCopyState()
...
function config.SelectBank.ButtonBankSelect:isCopySource()
...
function config.SelectBank.ButtonBankSelect:isCopyTarget()
    if self:isCopySource() then
        return false
    end
...
function config.SelectBank.ButtonBankSelect:updateState()
...
function config.SelectBank.ButtonBankSelect:action()
    if not self.handler:copyModeActive() then
